...
def Force_between_two_charges__F(m,n,o):
	return("F=",(float(m)*float(n)/float(o)**2)*9,"10^9F","N\nFormula:F=k×q1×q2/r^2")
...
def conductivity__sigma(m):
	return("σ=",float(m)*8.854*10**-12,"ohm^-1\nFormula:E=σ/ε0")
...
def Relation_between_R_and_ρ__R(m,n,o):
	return("R=",float(m)*float(o)/float(n),"ohm\nFormula:R=ρ×l/A")
def Resistivity__Rho(m,n,o):
	return("ρ=",float(m)*float(n)/float(o),"ohm metre\nFormula:ρ=R×A/l")
