        ...
        for i in ev:
            ...
            #    a = True
                ...
                try:
                    ...
                    print('exc')
                    e = Label(fram_run_fun,text = 'fill all values with digit')
                    if l_ans != None:
                        l_ans.destroy()
                    e.grid(row = u+2,column = 0)
                    a = False
                    break
        if a:
            find_ans()
...
def intro():
    ...
    Label(fram_intro,text = '',pady = 125).grid(row = 0,column = 0)
    but_for = Button(fram_intro,text = 'view formula to solve',command = view_for)
    but_for.grid(row = 1,column = 0,padx = 40,pady = 10)
    but_gr_ap = Button(fram_intro,text = 'graph and real life applications',command = view_gra_con)
    but_gr_ap.grid(row = 2,column = 0,padx = 40,pady = 10)
    but_hello = Button(fram_intro,text = '<not set up to now>',command = view_)
    but_hello.grid(row = 3,column = 0,padx = 40,pady = 10)
